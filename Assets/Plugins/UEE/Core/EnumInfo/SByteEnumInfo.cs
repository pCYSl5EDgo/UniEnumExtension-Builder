...
{
    public sealed class SByteEnumInfo : CompatibleIntEnumInfo
    {
        public SByteEnumInfo(TypeDefinition typeDefinition) : base(typeDefinition)
        {
            for (var i = 0; i < this.Values.Length; i++)
            {
                Values[i] = (sbyte) this.FieldDefinitions[i].Constant;
        ...
        private IEnumerable<NameValuePair<int>> TryAssign(byte b, IEnumerable<NameValuePair<int>> candidate)
        {
            ...
            {
                var value = (byte)(sbyte) Pairs[i].Value;
                if (value == default) break;
                if (b == value)
                    return candidate.Append(Pairs[i]);
                if ((b & value) == value)
                {
                    return TryAssign((byte)(sbyte) (b ^ value), candidate.Append(Pairs[i]));
                }
            }
